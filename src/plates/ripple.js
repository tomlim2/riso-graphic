...
export const ripple = {
  ...
  paint(S, R, page) {
    ...
    const { rings, ease: easeBase, arcs: arcCount, weight, squash, dot, offset } = knobs;
    ...
    if (dot > 0) {
      ...
      const size = dot * beat;
      const heading = R.around(Math.PI * 0.45, 0.35);
      const dx = (Math.cos(heading) * size * offset) / 2;
      const dy = (Math.sin(heading) * size * offset) / 2;
      const cut = { lobes: 3, wobble: 0.07, squash: 1.1 };
      S.key.shape(shapes.blob(R, cx - dx, cy - dy, size, cut));
      S.body.shape(shapes.blob(R, cx + dx, cy + dy, size, cut));
    }
  }
};
