...
모이는 물결로 보인다. 비율은 `고리 수 × 잡는 프레임 ÷ 48`이다.

| 계당 고리 | 초당 24장 | 초당 12장 | 초당 8장 |
| --- | --- | --- | --- |
| 12 | 0.25 | 0.50 | 0.75 |
| 4 | 0.08 | 0.17 | 0.25 |

0.5에 가까우면 방향이 흐려지고 넘으면 뒤집힌다. 그래서 계마다 고리를 서넛만 두고, 대신
계를 넷 놓아 밀도를 되찾았다. 계마다 간격이 다르므로 서로 짝지어지지 않는다. 고리마다
자리와 속도를 조금씩 흩은 것, 마루 뒤에 얇은 자취를 한 줄 더한 것도 같은 이유다 — 앞뒤가
다른 모양이면 눈이 자리가 아니라 모양으로 짝을 짓는다.

방향도 눈이 아니라 자로 본다. 계 중심에서 바깥으로 밝기를 훑어 이웃 두 프레임을 겹쳐
보고, 어느 쪽으로 밀었을 때 가장 잘 맞는지 찾는다. 양수면 퍼짐, 음수면 모임이다.

| 초당 장수 | 고친 전 | 고친 뒤 |
| --- | --- | --- |
| 24 | 바깥 17 / 안쪽 7 | 바깥 35 / 안쪽 0 |
| 12 | 바깥 2 / 안쪽 10 | 바깥 18 / 안쪽 1 |
| 8 | 바깥 5 / 안쪽 3 | 바깥 10 / 안쪽 0 |
