...
한 장을 두 프레임씩 잡아 둔다. 손그림 애니메이션이 늘 하던 투스 촬영이고, 움직임에 또렷한
박자가 생긴다. 한 바퀴는 48프레임, 2초다. 물결의 박자가 거기 맞춰져 있다.

값이 아니라 약속이라 다이얼이 없다. 바꿀 일이 있으면 `src/main.js`의 `HOLD` 한 줄이다.
1로 내리면 프레임마다 새 장이라 초당 스물네 장이 되고, 흐름이 매끄러워지는 대신 굽는
시간이 두 배가 된다.
...
| MOTION | 재생과 프레임 끌기. 박자와 루프 길이는 고정이라 다이얼이 없다 |
