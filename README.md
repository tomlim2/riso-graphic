...
프레임마다 새로 찍는다. 두 프레임에 한 장을 쓰는 투스 촬영은 손그림 애니메이션이 늘 하던
일이고 굽는 시간도 절반이지만, 그러면 그림은 초당 열두 장이다. 초당 스물네 번이라는 말과
다르므로 두지 않았다. 한 바퀴는 48프레임, 2초다. 물결의 박자가 거기 맞춰져 있다.
...
| MOTION | 재생과 프레임 끌기. 24fps와 2초 루프는 고정이라 다이얼이 없다 |
